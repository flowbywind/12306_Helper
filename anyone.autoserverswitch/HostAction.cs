...
#	::1             localhost";
        ...
        {
            File.Copy(this.Path, Environment.CurrentDirectory + "\\hosts",true);
        }
        ...
        {
            if (File.Exists(Environment.CurrentDirectory + "\\hosts"))
                File.Copy(Environment.CurrentDirectory + "\\hosts", this.Path, true);
        }
            ...
            lock (privateLocker)
            {
                if (!ExistEx(domain))
                {
                    try
                    {
                        using (StreamWriter sw = new StreamWriter(this.Path, true, Encoding.Default))
                        {
                            sw.WriteLine(String.Format("{0} {1}", ip, domain));
                        }
                    }
                    catch { RestoreHosts(); }
                }
                ...
                if (ExistEx(domain))
                {
                    try
                    {
                        ...
                        if (tmpstr != "")
                        {
                            ...
                            using (StreamWriter sw = new StreamWriter(this.Path, false, Encoding.Default))
                            {
                                sw.Write(strHost.Replace(tmpstr, String.Format("{0} {1}", ip, domain)));
                            }
                        }
                    }
                    catch { RestoreHosts(); }
